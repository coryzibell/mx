...
`kind` is `exact` (identical to a phrase after trimming), `close` (matched
only after normalizing case, punctuation or spelling) or `none`, and
`phrase_index` says which phrase matched. The bucket names what the tool did,
not what the responder knew.
...
advance the step, so the token the caller spent stops verifying, and both are
counted in `unjudged`.

The ritual is walked by the agent that began it: `MX_CURRENT_AGENT` must equal
the session's agent, and a caller naming no agent is refused as well.
...
A session created by a version of mx older than the one-guess ritual is refused
rather than continued. A completed session is kept, with `completed_at` set.

Naming the wrong entry is refused with `{status: "error", error:
"invalid_bloom_id", message, expected_id?}` on stderr and exit code 1, like
every other error.

=== The guess log

Every `--respond` that judges a guess writes one `wake_guess` row, keyed by
session and step, in the same transaction as the session advance: the agent, wake number, model, entry, chunk, both positions
in the sequence, the title as shown, the guess, the phrases it was matched
against, the match, the bucket, and the SHA-256 of the chunk text. If the
transaction fails, the respond call fails and neither the row nor the advance
lands.
...
counted too, although it would not have made the cut untagged.
