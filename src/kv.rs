...
impl KvStore {
    ...
    /// persists it via [`KvStore::save_schema`]. Accepts any of the five
    /// [`ValueType`]s and the full set of type-appropriate options.
    ///
    ...
    /// formatting -- acceptable and consistent with `add_key_def`.
    ///
...
"#
    ...
    fn rename_key_with_no_data_entries() {
        ...
        // default. Use a fresh history key via add_key_def instead.
        store
            .add_key_def("empty_history", empty_def(ValueType::History))
            .unwrap();
    ...
    #[test]
    fn add_key_def_hyphen_and_underscore_accepted() {
        let (mut store, _dir) = setup_store(test_schema());
        store
            .add_key_def("my-key_v2", empty_def(ValueType::History))
            .unwrap();
